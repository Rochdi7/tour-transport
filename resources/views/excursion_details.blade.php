...
@section('content')
    ...
    <div class="hero-section" data-aos="fade-up">
        <img loading="lazy"src="{{ asset('assets/images/excursions/' . $excursion['image']) }}" class="hero-image"
            alt="{{ $excursion['name'] }}">
        <div class="hero-overlay"></div>
        <div class="hero-content">
            <h1 style="color: white;">{{ $excursion['name'] }}</h1>
            <p class="duration-tag text-light">{{ $excursion['duration'] }}</p> <!-- Displays duration -->
            <div class="breadcrumb text-center">
                <a href="{{ url('/') }}" class="text-light">Accueil</a> »
                <a href="{{ url('/excursions') }}" class="text-light">Excursions</a> »
                <span style="color: var(--accent-color);">{{ $excursion['name'] }}</span>
            </div>
        </div>
    </div>
    ...
    <section class="container py-5">
        <div id="galleryCarousel" class="carousel slide" data-bs-ride="carousel">
            <div class="carousel-inner">
                @foreach ($excursion['gallery'] as $index => $image)
    ...
    <div class="container">
        ...
        <div id="programmeText" class="d-none" data-aos="fade-up">
            <div class="accordion" id="programmeAccordion">
                @foreach ($excursion['programme'] as $day => $info)
        ...
        <ul id="inclusText" class="list-group d-none" data-aos="fade-up">
            @foreach ($excursion['inclus'] as $item)
    ...
    <a href="https://wa.me/212770134539?text={{ urlencode('Bonjour, je souhaite réserver l\'excursion : ' . $excursion['name']) }}" 
   target="_blank" class="btn-whatsapp">
   <i class="fa-brands fa-whatsapp"></i> Réserver sur WhatsApp
</a>

    </div>

    <!-- Related Excursions Section -->
    @if (!empty($relatedExcursions))
    <section class="related-section py-5 bg-light">
        <div class="container">
            ...
            <div class="row">
                @foreach ($relatedExcursions as $related)
                    <div class="col-md-4 col-12 mb-4" data-aos="fade-up" data-aos-delay="200">
                        <a href="{{ route('excursions.show', ['slug' => Str::slug($related['name'])]) }}" class="excursion-card-link">
                            <div class="excursion-card">
                                <div class="excursion-image">
                                    <img loading="lazy"src="{{ asset('assets/images/excursions/' . $related['image']) }}" class="img-fluid"
                                        alt="{{ $related['name'] }}">
                                    <!-- Gallery Icon -->
                                ...
                                <div class="excursion-info p-3">
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <p class="excursion-transport"><i class="fas fa-bus"></i> {{ $related['transport'] }}</p>
                                    </div>
                                    <p class="excursion-description">{{ Str::limit($related['description'], 100) }}</p>
                                    <a class="custom-btn" href="{{ route('excursions.show', ['slug' => Str::slug($related['name'])]) }}" class="reserve-btn">
                                        Voir Détails
                                    </a>
                                </div>
                            </div>
